...
const SkillCard = ({ type, data }: Skills) => {
  ...
  return (
    <div className="border p-4 mx-2 rounded-2xl snap-mandatory flex flex-col flex-shrink-0 snap-center">
      ...
      {data.slice(0, 2).map((skill, skillIndex) => (
        <motion.div
          ...
          className="p-2 bg-gray-200 mb-2 rounded bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500"
        >
          <p className="text-darkBlack text-sm font-extralight">
            {skill.title}
          </p>
        </motion.div>
      ))}
